...
def actualizar_resultado():
    archivos_bert = glob(f'{RUTA_BASE}/ejecucion/bert_llm_*.parquet')
    if len(archivos_bert)>0:
        ...
        df_p.select(COLUMNAS_BERT
            ).write_parquet( f'{RUTA_BASE}/ejecucion/crudo_bert_llm.parquet')
    return df_p
...
df_stopwords = pl.read_excel(f'{RUTA_BASE}/limpieza/stopwords_llm.xlsx')
lst_llaves = sorted(list(dfs.keys()))
dfsw = {}

for modelo in lst_llaves:
    dfsw[modelo] = df_stopwords.filter(pl.col('llave')==modelo)
...
for i,llave in tqdm(enumerate(lst_llaves)):
    print(i,llave)
    columna_base = 'denuncia'
    tipo_df = llave
    df_ = dfs[llave]
    columnas = [columna_base]
        ...
        for porc_min_tamano_topico in [0.05, 0.01]:
            ...
            if hecho==0:
                ...
                df_topicos, modelo_bertopic = ejecutar_bertopic(dfs, 
                                                                ...
                                                                columna,
                                                                porc_min_tamano_topico)
                llave = f'{tipo_df}-{columna}-{porc_min_tamano_topico}'
                try:
...
df_palabras.write_parquet( f'{RUTA_BASE}/ejecucion/bert_LLM_{int(i)}.parquet')
