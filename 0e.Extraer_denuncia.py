...
def obtener_denuncia(contenido):
    ...
    for limite in lst_limites_2:
        ...
        for borde in lst_borde:
            if borde in contenido_min:
                denuncia = contenido_min.split(borde)[0]
                break
            else:
                borde = f'{limite}:\n'
                if borde in contenido_min:
                    ...
                    break
    if denuncia:
        contenido_cam = contenido_cam[:len(denuncia)]
        contenido_min = destildar(contenido_cam).lower()
        for limite in lst_limites_1:
            borde = f'{limite}\n'
            if borde in contenido_min:
                denuncia = contenido_min.split(borde)[1]
                break
            else:
                borde = f'{limite}:\n'
                if borde in contenido_min:
                    denuncia = contenido_min.split(borde)[0]
                    break
        denuncia = contenido_cam[-len(denuncia):]
    else:
        denuncia = None
    return denuncia
...
df_muestra = (df_muestra.with_columns(
        ...
        .map_elements(obtener_denuncia, return_dtype=pl.Utf8)
        .alias('denuncia_anonimizada')
    )
)
print('Sin denuncia:',df_muestra.filter(pl.col('denuncia_anonimizada').is_null()).shape[0])
...
df_muestra = actualizar_valores_nulos(df_muestra, 'denuncia_anonimizada')
...
df_muestra.write_parquet(f'{RUTA_BASE}/../temporal/datos/muestra_denuncias_2025.parquet')
